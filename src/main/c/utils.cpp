...
int my_rand(int low, int high) {
	...
	//return (int) (low + (high-(low)+1) * my_frand());
}
...
void my_srand(unsigned int seed) {
    srand(seed);
	//SCI2S_Seed = (unsigned long) seed;
}

//-----------------------------------------------------------------------

// Return a random float between 0 and 1 (excluded).
/* Genera un numero aleatorio real en el intervalo [0,1[
   (incluyendo el 0 pero sin incluir el 1) */
float my_frand() {
    return ((my_rand(0,RAND_MAX) * 1.) / (RAND_MAX * 1. + 1));
	//return (( SCI2S_Seed = ( (SCI2S_Seed * SCI2S_PRIME) & SCI2S_MASK) ) * SCI2S_SCALE );
}

//-----------------------------------------------------------------------

clock_t chrono_state() {
	return clock();
}
...
double chrono_diff(clock_t begin, clock_t end) {
